...
const ensureSensor = async (sensor) => {
    ...
    if (check.rowCount === 0) {
        await query(
            'INSERT INTO sensors (id, name, type, unit, threshold) VALUES ($1, $2, $3, $4, $5)',
            [sensor.id, sensor.name, sensor.type, sensor.unit, sensor.threshold]
        );
    }
};
...
const saveReading = async (sensorId, value, status) => {
    try {
        await query(
            'INSERT INTO readings (sensor_id, value, status) VALUES ($1, $2, $3)',
            [sensorId, value, status]
        );
    } catch (err) {
...
const createAlert = async (sensorId, type, message) => {
    try {
        ...
        const check = await query(
            "SELECT id FROM alerts WHERE sensor_id = $1 AND type = $2 AND status = 'active'",
            [sensorId, type]
        );
        ...
        if (check.rowCount === 0) {
            ...
            const res = await query(
                "INSERT INTO alerts (sensor_id, type, message, status) VALUES ($1, $2, $3, 'active') RETURNING *",
...
const autoResolveAlerts = async (sensorId) => {
    try {
        const res = await query(
            "UPDATE alerts SET status = 'resolved', resolved_at = NOW() WHERE sensor_id = $1 AND status = 'active' RETURNING *",
            [sensorId]
        );
        if (res.rowCount > 0) {

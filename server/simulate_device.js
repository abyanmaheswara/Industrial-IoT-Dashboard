...
client.on("connect", () => {
  ...
  setInterval(() => {
    ...
    if (sensorId === "temp_01")
      value = (20 + Math.random() * 60).toFixed(1); // 20-80 C
    else if (sensorId === "press_01")
      value = (1 + Math.random() * 9).toFixed(1); // 1-10 Bar
    else if (sensorId === "vib_01")
      value = (Math.random() * 50).toFixed(2); // 0-50 Hz
    ...
    const topic = `factory/1/sensors/${sensorId}`;

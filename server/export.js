...
async function getReportData() {
  ...
  const sensorReadings = await Promise.all(
    sensors.map(async (sensor) => {
      ...
      // db.getHistory already reverses for charts (oldest last),
      // but here we want the absolute latest which is the LAST element in the reversed array or FIRST in raw query.
      // Actually, db.getHistory does .reverse(), so the 'latest' is the last element.
      const latest = history[history.length - 1] || { value: 0, status: "unknown", timestamp: new Date() };
      ...
      return {
        ...
        unit: sensor.unit,
        status: (latest.status || "NORMAL").toUpperCase(),
  ...
  return {
    ...
    alerts: alerts.slice(0, 50).map((a) => ({
      id: a.id,
      severity: (a.type || "INFO").toUpperCase(),
      sensorName: a.sensor_name || a.sensor_id,
      message: a.message,
...
const exportPDFReport = async (res) => {
  try {
    ...
    stats.forEach((stat) => {
      doc.fontSize(10).fillColor("#666666").text(stat.label, 70, doc.y, { width: 200, continued: true }).fillColor("#333333").text(`: ${stat.value}`, { align: "left" });
    });
    ...
    // ===== ANOMALY LOG TABLE =====
    doc.addPage();
    doc.fillColor(bronze).fontSize(16).text("Anomaly & Incident Log", { underline: true });
    doc.moveDown(0.8);

    const alertHeaders = ["Timestamp", "Severity", "Node", "Message"];
    const alertWidths = [100, 70, 100, 225];
    ...
    data.alerts.forEach((alert) => {
      ...
      const rowY = doc.y;
      doc.fontSize(8).fillColor("#666666").text(alert.timestamp, 50, rowY, { width: 90 });

      const sevColor = alert.severity === "CRITICAL" ? "#ef4444" : "#f59e0b";
      doc.fillColor(sevColor).font("Helvetica-Bold").text(alert.severity, 150, rowY);

      doc.fillColor("#333333").font("Helvetica").text(alert.sensorName, 220, rowY, { width: 90 });
      doc.text(alert.message, 320, rowY, { width: 220 });
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc
        .fontSize(8)
        .fillColor("#999999")
        .text(`Page ${i + 1} of ${pageCount} | Cybernetically Generated Report`, 50, doc.page.height - 50, { align: "center" });
    }
